/* cornea_tracing_plugin.cpp
 * This is a cornea tracing plugin which traces ommatidia images using the following steps:
 1) adaptive image thresholding
 2) histogram equalization
 3) rescaling (direct thresholding for now)
 4) SIGEN neuron tracing
*/
...
bool ExamplePlugin::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
	...
	else if (func_name == tr("full_tracing"))
	{

		//unsigned char * pData;
		full_tracing(callback,input,output);
	}
